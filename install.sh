...
install_fluent_bit()
{
    ...
    tar -zxvf fluentbit.tar.gz >/dev/null && mv -f fluent-bit /opt/td-agent-bit/bin/td-agent-bit && mv -f GeoLite2-City.mmdb $TDAGENTCONFDIR && mv -f ua-parser $TDAGENTCONFDIR && mv -f uaparser /opt/td-agent-bit/bin/ 
